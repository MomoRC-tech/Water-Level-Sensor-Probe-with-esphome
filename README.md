...
### Entity Summary (User-Facing)
- Sensor Power (switch): Relay control powering sensor loop.
- Stay Awake (switch): Prevents deep sleep for OTA & debugging.
- Depth Below Head (`water_depth_from_head`): Water surface depth below well head.
- Depth Below Surface (`water_depth_from_surface`): Water surface depth below ground surface.
- Over Pump 1 (`water_over_pump1`): Positive = water above pump 1 intake.
- Over Pump 2 (`water_over_pump2`): Positive = water above pump 2 intake.
- Loop Current (`loop_current_filtered`): Filtered 4–20 mA loop measurement.
- Sensor Error (`water_sensor_error`): True if shunt voltage out of plausible range.

### Diagnostic (Internal) Entities
- ADC Internal (`shunt_adc_raw`): Raw 0–1 V ADC reading.
- Shunt Voltage Raw (`shunt_voltage`): Calculated actual voltage across shunt.
- Loop Current Raw (`loop_current_raw`): Unfiltered current (NaN if invalid / <3.9 mA).
- Sensor Column (`sensor_column_filtered_m`): Water column height above sensor (0–span).
- Loop Current (filtered) (`loop_current_filtered`): Filtered current (also user-facing).
